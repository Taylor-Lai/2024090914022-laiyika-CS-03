...
#### 2.
      ...
      2,内存大小和空间
      栈区：栈的大小系统预先规定好的，空间相对较小。这是一个编译时就确定的常数。如果申请的空间超过栈的剩余空间，将提示栈溢出。
...
```
这里我用的是递归的办法，也就是返回值里仍然套用了这个函数，这就产生了很多的数据，这些数据无一例外存储在栈区上，因为函数没有结束，所以一直没有被清理。如果我输入的1000000，那么就会出现错误，也就是爆栈。
...
这里稍稍更换了方式，大体差不多，但是变成了反复对于结果进行赋值，所以，归根结底只调用了几个变量，避免了爆栈的危险。
